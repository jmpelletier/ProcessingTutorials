  ...
  if (cam.available() == true) {
    ...
    for (int y = 0; y < cameraImage.height; y++) {
      for (int x = 0; x < cameraImage.width; x++) {
        ...
        // RGBのディレイを別々に計算する
        float delayR = map(sin(10 * (float)y / cameraImage.height), -1.f, 1.f, 0.f, bufferSize - 1);
        float delayG = map(sin(9 * (float)y / cameraImage.height), -1.f, 1.f, 0.f, bufferSize - 1);
        float delayB = map(sin(8 * (float)y / cameraImage.height), -1.f, 1.f, 0.f, bufferSize - 1);
