...
PGraphics patternGraphics; // パターンの画像
...
void setup() {
  ...
  // パターンを描写する
  patternGraphics.beginDraw();
  patternGraphics.background(0);
  patternGraphics.noStroke();
  int w = width / 64;
  for (int i = 0; i < 64; i++) {
    patternGraphics.fill(random(255));
    patternGraphics.rect(i * w, 0, w, height);
  }
  patternGraphics.endDraw();
  patternGraphics.loadPixels();
  patternPixels = patternGraphics.pixels;
  ...
  if (cam.available() == true) {
    ...
    for (int y = 0; y < cameraImage.height; y++) {
      for (int x = 0; x < cameraImage.width; x++) {
        ...
        // ここでディレイのフレーム数を計算する
        // 赤色だけを取得して、ディレイ時間に換算する
        int delay = (int)map(red(patternPixels[i]), 0, 255, 0, bufferSize - 1);
