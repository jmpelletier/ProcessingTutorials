...
void setup() {
  ...
  // 外部の画像を読み込む
  patternImage = loadImage("candies.jpg");
  // 念のために、画像のサイズを合わせる
  patternImage.resize(width, height);
  // 画像のピクセルを取得する
  patternImage.loadPixels();
  patternPixels = patternImage.pixels;
  ...
  if (cam.available() == true) {
    ...
    for (int y = 0; y < cameraImage.height; y++) {
      for (int x = 0; x < cameraImage.width; x++) {
        ...
        // RGBのディレイを別々に計算する
        float delayR = map(red(patternPixels[i]), 0, 255, 0.f, bufferSize - 1);
        float delayG = map(green(patternPixels[i]), 0, 255, 0.f, bufferSize - 1);
        float delayB = map(blue(patternPixels[i]), 0, 255, 0.f, bufferSize - 1);

        // ディレイされた色を3つ取得する
        color c1 = getPixel(i, delayR);
        color c2 = getPixel(i, delayG);
        color c3 = getPixel(i, delayB);
